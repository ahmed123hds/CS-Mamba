...
try:
    from models.characteristic_mamba_v6_1 import CSMamba_V6_1
except ImportError:
    from characteristic_mamba_v6_1_fixed import CSMamba_V6_1
...
def parse_args():
    ...
    p.add_argument("--n_v6_layers", type=int, default=2, help="Number of late-stage V6 layers in the stagewise hybrid")
...
def _mp_fn(index, flags):
    ...
    cfg.n_v6_layers = flags.n_v6_layers

    model = CSMamba_V6_1(cfg).to(device)
    ...
    xm.master_print(f"\n{'='*72}")
    xm.master_print(f"CS-Mamba V6.1 (Stagewise Hybrid) | Params: {n_params/1e6:.1f}M")
    ...
    for epoch in range(start_epoch, flags.epochs + 1):
        ...
        latest_path = os.path.join(flags.save_dir, f"csmamba_v6_1_{flags.dataset}_latest.pt")

        if xm.is_master_ordinal():
            xm.save(ckpt, latest_path)

        if abs(v_acc - best_acc) < 1e-12:
            best_path = os.path.join(flags.save_dir, f"csmamba_v6_1_{flags.dataset}_best.pt")
            if xm.is_master_ordinal():
