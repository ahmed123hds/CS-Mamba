...
try:
    ...
    # Fallback for local/standalone execution
    from continuous_spatial_mamba_v4 import ContinuousSpatialMambaBlock_V4
    from characteristic_mamba_v6 import CharacteristicMambaBlock_V6
...
class CSMamba_V6_1(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.v4_layers = nn.ModuleList(layers[:n_v4_layers])
        self.v6_layers = nn.ModuleList(layers[n_v4_layers:])
        self.layers = nn.ModuleList(layers)  # kept for introspection/compatibility
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for layer in self.v4_layers:
            x = layer(x, K_steps=self.k_steps)
        for layer in self.v6_layers:
            x = layer(x, k_steps=self.k_steps)
